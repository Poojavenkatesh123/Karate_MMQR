Feature: Inactivate Merchant API
...
Scenario: Verify API returns 200 status
    Given path '/merchant-onboard/v1/onboarding/deactivating-merchant/00959758365232'
    When method PATCH
    Then status 200

Scenario: Verify merchant deactivated successfully for valid mobile number
    Given path '/merchant-onboard/v1/onboarding/deactivating-merchant/00959758365232'
    When method PATCH
    Then status 200
    And match response.status == 'IN_ACTIVE'
    And match response.message == 'Merchant deactivated successfully'

  Scenario: Verify error response when merchant does not exist in system
    Given path '/merchant-onboard/v1/onboarding/deactivating-merchant/0959758365233'
    When method PATCH
    Then status 500
  ...
  Scenario: Verify error response when no mobile number is passed in path
    Given path '/merchant-onboard/v1/onboarding/deactivating-merchant/'
    When method PATCH
    Then status 404
    ...
    And match response.errorDescription == 'No static resource v1/onboarding/deactivating-merchant.'
    And match response.timestamp != null
